...
النسخة الحالية تعمل كـ SaaS web prototype قابل للتجربة على السيرفر:

- تسجيل ودخول السوبر أدمن والتاجر.
- إنشاء متجر متعدد المستأجرين مع عزل البيانات بـ `tenant_id`.
- إدارة التصنيفات والمنتجات، نشر/إخفاء/حذف المنتج، وربط المنتج بتصنيف.
- واجهة متجر عامة بالـ slug مع سلة وCheckout.
- إنشاء الطلبات، خصم المخزون، متابعة تفاصيل الطلب وتغيير حالته من لوحة التاجر.
- اشتراكات التاجر للمنصة عبر Paymob الخاص بالسوبر أدمن.
- Paymob للتاجر لتحصيل طلبات المتجر.
- نشر تلقائي إلى TrueNAS عبر GitHub Actions + Tailscale.

النواقص قبل الاعتماد التجاري الكامل:

- نقل الواجهة النهائية إلى Flutter Web/Mobile أو تطوير frontend production منفصل.
- إضافة اختبارات آلية وقفل إصدارات الحزم بـ lockfile.
- ضبط أسرار الإنتاج في `.env` وعدم استخدام أي قيم placeholder.
- تشغيل النسخ الاحتياطي دوريًا ونقل النسخ إلى تخزين خارجي.
- نقل الواجهة النهائية إلى Flutter Web/Mobile عندما تستقر كل flows.

### نسخ احتياطي سريع

على السيرفر داخل `DEPLOY_PATH`:

```bash
BACKUP_DIR=/root/easy-shope-backups ./scripts/backup-postgres.sh
```

السكريبت يستخدم `pg_dump` من حاوية PostgreSQL ويخرج ملف `sql.gz`. انقل هذه الملفات دوريًا إلى تخزين خارجي أو Dataset آخر على TrueNAS.
